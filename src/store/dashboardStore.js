...
export const useDashboardStore = create()((set) => ({
  ...
  refreshStats: async () => {
    ...
    try {
      ...
      const pendingKyc = franchises.filter(
        (franchise) => franchise.kycStatus === "Pending"
      ).length;
      const activeFranchises = totalFranchises - pendingKyc;

      const totalCommission = franchises.reduce(
        (sum, franchise) => sum + franchise.commission,
        0
      );
      const averageCommission =
        totalFranchises > 0
          ? Number((totalCommission / totalFranchises).toFixed(2))
          : 0;

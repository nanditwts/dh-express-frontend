...
export const ChannelListPage: React.FC = () => {
  ...
  const handleEditSubmit = (data: Record<string, any>) => {
    ...
    // Simulate API call
    setTimeout(() => {
      updateChannel(selectedChannel.id, data as any);
    ...
    <Layout>
      ...
      >
        ...
        <Card>
          ...
          <Table
            columns={[
              { header: 'Courier', accessor: (item) => getCourierName(item.courierId) },
              { header: 'Business Name', accessor: 'businessName' },
              { header: 'Type', accessor: (item) => (
                <Badge variant={item.type === 'B2B' ? 'primary' : 'secondary'}>
                  {item.type}
                </Badge>
              )},
        ...
        >
          <div className="mb-6">
            <p className="text-gray-700">
              ...
              <span className="font-medium">{selectedChannel?.businessName}</span>?

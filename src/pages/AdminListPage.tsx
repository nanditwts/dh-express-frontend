...
export const AdminListPage: React.FC = () => {
  ...
  const filteredAdmins = admins.filter((admin) => {
    ...
    return (
      admin.businessName.toLowerCase().includes(searchValue) ||
  ...
  const handleEditSubmit = (data: Record<string, any>) => {
    ...
    // Simulate API call
    setTimeout(() => {
      updateAdmin(selectedAdmin.id, data as any);
  ...
  const getFranchiseCount = (adminId: string) => {
    ...
    <Layout>
      ...
      >
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-6">
          <div>
            <h1 className="text-2xl font-bold text-gray-900">Franchise Admins</h1>
        ...
        <Card>
          ...
          <Table
            columns={[
              ...
              { header: 'Phone', accessor: 'contactPersonMobile' },
              { header: 'City', accessor: 'city' },
              { header: 'Commission', accessor: (item) => `${item.commission}%` },
              { header: 'Franchises', accessor: (item) => (
                <Badge variant="primary">
                  {getFranchiseCount(item.id)}
                </Badge>
              )},

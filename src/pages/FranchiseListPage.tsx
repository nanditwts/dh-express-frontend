...
export const FranchiseListPage: React.FC = () => {
  const { franchises, addFranchise, updateFranchise, deleteFranchise } = useFranchiseStore();
  ...
  const filteredFranchises = franchises.filter((franchise) => {
    ...
    return (
      franchise.franchiseName.toLowerCase().includes(searchValue) ||
      franchise.contactPersonName.toLowerCase().includes(searchValue) ||
      franchise.city.toLowerCase().includes(searchValue) ||
      franchise.state.toLowerCase().includes(searchValue) ||
      franchise.gstNumber.toLowerCase().includes(searchValue)
    );
  });
  ...
  const handleEditSubmit = (data: Record<string, any>) => {
    ...
    // Simulate API call
    setTimeout(() => {
      updateFranchise(selectedFranchise.id, data as any);
        ...
        <Card>
          ...
          <Table
            columns={[
              ...
              { header: 'Admin', accessor: (item) => getAdminName(item.adminId) },
        ...
        >
          <div className="mb-6">
            <p className="text-gray-700">
              ...
              <span className="font-medium">{selectedFranchise?.franchiseName}</span>?
